...
export const PMForm = () => {
  ...
  const [formData, setFormData] = useState({ name: "", email: "", phone: "", role: "founder", brand_url: "", ad_spend: "" });
  const [loading, setLoading] = useState(false);
  const [submitted, setSubmitted] = useState(false);

  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!formData.name || !formData.email || !formData.phone || !formData.brand_url) {
    ...
    try {
      const res = await api.post("/session/lead", {
        ...
        role: `Brand: ${formData.brand_url} | Spend: ${formData.ad_spend}`, 
        // We pack brand_url and ad_spend into role for now to reuse the existing backend model without DB changes, 
        // or we can just send it and if the backend ignores it, it's fine.
      });
  ...
  return (
    <section id="apply" className="py-20 sm:py-32 bg-ink border-t border-line overflow-hidden">
      <div className="max-w-xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
        <div className="mb-10">
          ...
          <p className="text-white/70">
            We are currently onboarding a strict maximum of 3-4 high-end D2C brands. Tell us about your brand.
          </p>
        </div>
        ...
        {!submitted ? (
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="email" required value={formData.email} onChange={e => setFormData(f => ({ ...f, email: e.target.value }))} className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-white focus:border-blue outline-none transition-colors" placeholder="founder@brand.com" />
                </div>
                <div>
              ...
              <div>
                ...
                <input type="url" required value={formData.brand_url} onChange={e => setFormData(f => ({ ...f, brand_url: e.target.value }))} className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-white focus:border-blue outline-none transition-colors" placeholder="https://yourbrand.com" />
              </div>

              <div>
                <label className="block text-[11px] font-bold text-white/50 uppercase tracking-widest mb-1.5">Monthly Ad Spend</label>
                <select value={formData.ad_spend} onChange={e => setFormData(f => ({ ...f, ad_spend: e.target.value }))} className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-white focus:border-blue outline-none transition-colors appearance-none">
              ...
              <p className="text-center text-xs text-white/40 mt-4">We will review your application and contact you within 24 hours.</p>
            </form>
          </div>
        ) : (
          <div className="bg-white/5 border border-white/10 rounded-2xl p-8 sm:p-12">
            ...
            <p className="text-white/60">Thank you for your interest. We'll be in touch soon.</p>
          </div>
        )}
      </div>
    </section>
